...
font-weight:700;FONT-SIZE: 15px;height:70px;margin-bottom:20px;">
         ...
         <div class="card">
             <div class="card-body">
                 ...
                 <div class="row ">
                     ...
                     @include('tenant.Curriculum.paper_maker.partials.select_mcq')
                     @endif
                     @if (!empty($fill_in_the_blanks_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_fill_in_the_blank')
                     @endif
                     @if (!empty($true_and_false_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_true_and_false')
                     @endif
                     @if (!empty($column_matching_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_column_matching_question')
                     @endif
                     @if (!empty($short_question_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_short_question')
                     @endif
                     @if (!empty($words_and_use_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_words_and_use_question')
                     @endif
                     @if (!empty($select_words_and_use_question_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_select_words_and_use_question')
                     @endif
                     @if (!empty($paraphrase_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_paraphrase_question')
                     @endif
                     @if (!empty($stanza_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_stanza_question')
                     @endif
                     @if (!empty($passage_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_passage_question')
                     @endif

                     @if (!empty($long_question_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_long_question')
                     ...
                     @include('tenant.Curriculum.paper_maker.partials.select_translation_to_english_question')
                     @endif
                     @if (!empty($translation_to_urdu_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_translation_to_urdu_question')
                     @endif
                     @if (!empty($grammar_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_grammar_question')
                     @endif
                     @if (!empty($contextual_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_contextual_question')
                     @endif
                     @if (!empty($singular_and_plural_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_singular_and_plural_question')
                     @endif
                     @if (!empty($masculine_and_feminine_questions))
                     @include('tenant.Curriculum.paper_maker.partials.select_masculine_and_feminine_question')
                     @endif
                 </div>
                 <div class="d-lg-flex align-items-center mt-4 gap-3">
                     <div class="ms-auto">
 ...
 @section('javascript')
 <script src="{{ url('tenant/js/paper.js') }}"></script>
