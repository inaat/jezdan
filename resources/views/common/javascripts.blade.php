...
<script src="{{ url('tenant/js/vendor.js') }}" data-pagespeed-no-defer></script>
<script src="{{ url('tenant/js/sidebarapp.js') }}" ></script>

@if(file_exists(public_path('js/lang/' . session()->get('user.language', config('app.locale')) . '.js')))
    <script src="{{ url('tenant/js/lang/' . session()->get('user.language', config('app.locale') ) . '.js') }}"></script>
@else
    <script src="{{ url('tenant/js/lang/en.js') }}"></script>
@endif
...
if(sidebarcolor){
  ...
  $('html').attr('class', side);
    ...
    var financial_year = {
      ...
      var mathcss="{{ url('tenant/js/tinymce/matheditor/html/css/math.css')}}";
...
@else
    <script src="{{ url('tenant/js/lang/en.js') }}"></script>
@endif
		<script src="{{ url('tenant/js/tinymce/tinymce.js') }}"></script>
		<script src="{{ url('tenant/js/tinymce/matheditor/plugin.js') }}"></script>

<script src="{{ url('tenant/js/functions.js') }}" data-pagespeed-no-defer></script>
<script src="{{ url('tenant/js/common.js') }}" data-pagespeed-no-defer></script>
<script src="{{ url('tenant/js/whatsapp.js') }}" data-pagespeed-no-defer></script>
<script src="{{ url('tenant/js/apps.js') }}"></script>
<script src="{{ url('tenant/js/u2.js') }}"></script>


<!-- TODO -->
@if(file_exists(public_path('AdminLTE/plugins/select2/lang/' . session()->get('user.language', config('app.locale')) . '.js')))
    <script src="{{ url('tenant/AdminLTE/plugins/select2/lang/' . session()->get('user.language', config('app.locale') ) . '.js') }}"></script>
@endif
@php
...
@if(file_exists(public_path() . '/js/jquery-validation-1.16.0/src/localization/' . $validation_lang_file))
    <script src="{{ url('tenant/js/jquery-validation-1.16.0/src/localization/' . $validation_lang_file . '') }}"></script>
@endif
