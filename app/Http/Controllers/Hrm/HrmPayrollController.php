...
if (auth()->user()->can('hrm_payment.view')) {
    $html .= '<li><a href="' . action('Hrm\HrmTransactionPaymentController@show', [$row->id]) . '" class="dropdown-item view_payment_modal"><i class="fas fa-money-bill-alt"></i> ' . __("english.view_payments") . '</a></li>';
        ...
        ->editColumn(
            ...
            function ($row) {
                ...
                return (string) view('tenant.hrm.payroll.partials.payment_status', ['payment_status' => $payment_status, 'id' => $row->id]);
            }
        )
        ...
        return view('tenant.hrm.payroll.index')->with(compact('campuses','sessions'));
    }
    ...
    public function create()
    {
        ...
        return view('tenant.hrm.payroll.create')->with(compact('campuses', 'now_month'));
    }
    public function payrollAssignSearch(Request $request)
    {
        ...
        return view('tenant.hrm.payroll.payroll_assign')->with(compact('employees', 'status', 'campuses', 'campus_id', 'month_year', 'month_name', 'transaction_date', 'year', 'deductions', 'allowances'));
    }
    /**
    ...
    public function edit($id)
    {
        ...
        return view('tenant.hrm.payroll.edit')->with(compact('transaction', 'deductions', 'allowances'));
    }
    ...
    */
    ...
    private function receiptContent($transaction)
    {
        ...
        $output['html_content'] = view('tenant.hrm.payroll.pay_slip', compact('transaction','employee_details'))->render();
