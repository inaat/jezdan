...
class FeeCardPrintController extends Controller
{
    ...
    public function createClassWisePrint(Request $request)
    {
        ...
        return view('tenant.school-printing.feecard.create')->with(compact('campuses', 'now_month'));
    }
    ...
    private function generateFeeCard($student)
    {
        ...
        $snappy = \WPDF::loadview('tenant.school-printing/feecard.class-wise-fee-card', compact('student'));
    ...
    private function receiptContent($student)
    {
        ...
        $receipt_details = [];
        //$output['html_content'] = view('tenant.school-printing.feecard.single_fee_card', compact('student'))->render();
        $output['html_content'] = view('tenant.school-printing.feecard.fee_card_template1', compact('student'))->render();
    ...
    private function receiptContentFee($student)
    {
        ...
        $output['is_enabled'] = true;
        $receipt_details = [];
        $output['html_content'] = view('tenant.school-printing.feecard.abasyn', compact('student'))->render();
