...
class TeacherDashboardController extends Controller
{
    public function index()
    {
        ...
        if ($user->user_type == 'teacher') {
            ...
            return view('tenant.teacher_layouts.teacher_dashboard.index')->with(compact('timetables'));
        } else {
    ...
    public function show($id)
    {
       ...
       // dd(request()->input('class_id'));
        ...
        if (!empty($ClassSubjects)) {
            ...
            return view('tenant.Curriculum.manage_subject.show')->with(compact('class_subject','ClassSubjects', 'chapters'));
        } else {
            abort(403, 'Unauthorized action.');
        }
    }
    ...
    public function teacherSubjectMarkEntry()
    {
        ...
        if ($user->user_type == 'teacher') {
            ...
            return view('tenant.teacher_layouts.teacher_dashboard.teacher_mark_entry')->with(compact('campuses', 'sessions'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        ,'campuses','session','current_class','current_class_section','exam_create','exam_create.term','subject_name','teacher'])
            ...
            // dd($json_grades[0]['id']);
            return view('tenant.teacher_layouts.teacher_dashboard.mark_entry')->with(compact('campuses', 'sessions', 'classes', 'sections', 'grades', 'json_grades', 'terms', 'campus_id', 'class_id', 'exam_create_id', 'class_section_id', 'session_id', 'subject_id', 'classSubject', 'subs'));
        } else {
            return redirect('/dashboard');
        }
    }
    ...
    public function teacher_subject_marks_print()
    {
        ...
        if ($user->user_type == 'teacher') {
            ...
            return view('tenant.teacher_layouts.teacher_dashboard.create_mark_print')->with(compact('campuses', 'sessions'));
        } else {
            return redirect('/dashboard');
        }
    }
    ...
    public function teacherRoutineTestSubjectMarkEntry()
    {
        ...
        if ($user->user_type == 'teacher') {
            ...
            $campuses= $this->get_teacher_campus($teacher_id);
            $sessions=Session::forDropdown(false, true);
            return view('tenant.teacher_layouts.teacher_dashboard.teacher_routine_test_mark_entry')->with(compact('campuses', 'sessions'));
        } else {
            return redirect('/home');
        }
    }
